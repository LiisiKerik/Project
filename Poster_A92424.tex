  ...
  \begin{columns}
    \column{0.5}
      \block{The Algorithm}{
        ...
        \end{thebibliography}}
      \block{Precision}{
        For each algorithm and matrix size, we performed $10^4$ precision measurements on random matrices with elements in $[-1,1]$ and took the median error. Precision was measured by comparing the outcomes of floating-point computations to the outcomes of exact computations on fractions.
    ...
    \column{0.5}
      \block{Performance}{
        For each algorithm and matrix size, we measured how many random matrices with elements in $[-1,1]$ can be processed in one second and took the average running time.
      ...
      \block{Interesting Features, Advantages and Uses}{
        \begin{itemize}
          ...
          \item
            The algorithm is division-free. This is an advantage when we are working in a ring where not all elements have inverses, or when division is slow or imprecise.
          \item
